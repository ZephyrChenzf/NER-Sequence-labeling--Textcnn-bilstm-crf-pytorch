...
class BILSTM_CRF(nn.Module):
    ...
    def neg_log_likelihood(self,feats,tags,mask):
        #feats:  bilstm#100,b,10
        batch_size=feats.size(1)
        all_score=self.get_all_score(feats,mask)#
        real_score=self.get_real_score(feats,mask,tags)#b
    ...
    def viterbi_decode(self, emissions,mask):
        ...
        length_mask = torch.sum(mask, dim=1).view(batch_size, 1).long()  # b,1
        mask=mask.permute(1,0).contiguous().float()#s,b

        viterbi_history=[]
        viterbi_score = self.start_transitions.view(1, -1) + emissions[0]  # b,m,
        ...
        for i in range(1, seq_length):
            ...
            best_score,best_path = torch.max(score, 1)  # b,m
        ...
        last_position = (length_mask.contiguous().view(batch_size, 1, 1).expand(batch_size, 1, num_tags) - 1).contiguous()  # b,1->b,1,m
        ...
        decode_idx = Variable(torch.LongTensor(seq_length, batch_size))#s,b
        if use_cuda:
    ...
    def forward(self, feats,mask):
        #feats    #bilstm#100.b.10
        best_path=self.viterbi_decode(feats,mask)#b,s
        return best_path
